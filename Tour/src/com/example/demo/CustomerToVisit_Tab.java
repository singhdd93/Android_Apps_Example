...
public class CustomerToVisit_Tab extends Activity {
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		final TextView pmt = new TextView(this);
		pmt.setId(20);
		pmt.setText("Pmt");
		pmt.setTextColor(Color.WHITE);
		pmt.setPadding(5,5,5,5);
		tour_details_head.addView(pmt);// add the column to the table row here
		pmt.setTextSize(TypedValue.COMPLEX_UNIT_SP, 12);    
		...
		pgLogin = new ProgressDialog(CustomerToVisit_Tab.this);
	...
	private class MyAsyncTaskForTableBody extends AsyncTask<String, Integer, Double>{
		...
		protected void onPostExecute(Double result){
			...
			else
			{
				...
				Toast.makeText(CustomerToVisit_Tab.this, "Sorry. Internet Connection problem.", Toast.LENGTH_LONG).show();
			}
		}
		...
		public void postData(String username,String pwd,String Type,String City,String TourID,String Dt) {
			...
			HttpPost httppost = new HttpPost("http://61.8.158.157:89/ServiceTour.asmx/CustomerToVisit");
	...
	private void processCustomerToVisiteResponce(String responceFromServer) 
	{
		try {
			...
			FileOutputStream fOut = openFileOutput("processCustomerToVisiteResponceData.xml",MODE_WORLD_READABLE);
			...
			FileInputStream fIn = openFileInput("processCustomerToVisiteResponceData.xml");
			...
			{
				NodeList CustomerToVisit =doc.getElementsByTagName("CustomerToVisit");


				for (int i=0;i<CustomerToVisit.getLength();i++) 
				{
					String Visit		 = "" + ((Element)CustomerToVisit.item(i)).getAttribute("Visit");
					String Pmt  		 = "" + ((Element)CustomerToVisit.item(i)).getAttribute("Pmt");	
					String Visited    	 = "" + ((Element)CustomerToVisit.item(i)).getAttribute("Visited");
					String Tm 			 = "" + ((Element)CustomerToVisit.item(i)).getAttribute("Tm");	
	...
	private void createTableBody(String Visit, String Pmt, String Visited, String Tm, String PersonVisited, String Remarks, String CS_NAME, String csCode, final String ReportLink) 
	{
		...
		final CheckBox pmt = new CheckBox(this);
		pmt.setId(20);
		pmt.setTextColor(Color.BLACK);
		pmt.setTextSize(TypedValue.COMPLEX_UNIT_SP, 12);
		customerToCatchTblRow.addView(pmt);// add the column to the table row here
		pmt.setEnabled(false);
		
		if(Pmt.equalsIgnoreCase("False"))
		{
			pmt.setChecked(false);
		}
		else if(Pmt.equalsIgnoreCase("True"))
		{
			pmt.setChecked(true);
		}
	...
	public void SaveData(View v) {
		...
		for(int i = 1; i < customerToCatchTbl.getChildCount(); i++)
		{
            ...
            CheckBox pmt      		= (CheckBox) row1.getChildAt(0);
            CheckBox visit   		= (CheckBox) row1.getChildAt(1);
            TextView customer  		= (TextView) row1.getChildAt(2);
            CheckBox visited   		= (CheckBox) row1.getChildAt(3);
	...
	private class MyAsyncTaskForSaveingCustomerToVisit extends AsyncTask<String, Integer, Double>{
		...
		protected void onPostExecute(Double result){
			...
			else
			{
				...
				Toast.makeText(CustomerToVisit_Tab.this, "Not getting proper responce from server.\nCould be wifi problem or server.", Toast.LENGTH_LONG).show();
			}
		...
		public void postData(String username, String pwd, String Type, String TourID, String City, String Dt, String ParDetailsToSend) {
			// Create a new HttpClient and Post Header
			HttpClient httpclient = new DefaultHttpClient();
			HttpPost httppost = new HttpPost("http://61.8.158.157:89/ServiceTour.asmx/CustomerToVisitEntry");
	...
	private void processCustomerToVisitResponce(String responceFromServer) 
	{
		try {
			//saving the file as a xml
			FileOutputStream fOut = openFileOutput("processCustomerToVisitResponce.xml",MODE_WORLD_READABLE);
			OutputStreamWriter osw = new OutputStreamWriter(fOut);
			osw.write(responceFromServer);
			osw.flush();
			osw.close();
			...
			//reading the file as xml
			FileInputStream fIn = openFileInput("processCustomerToVisitResponce.xml");
			InputStreamReader isr = new InputStreamReader(fIn);
			char[] inputBuffer = new char[responceFromServer.length()];
			isr.read(inputBuffer);
			String readString = new String(inputBuffer);
